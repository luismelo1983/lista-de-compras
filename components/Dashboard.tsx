...
import { IconPlus, IconTrash, IconChevronUp, IconChevronDown, IconSettings } from './Icons';
...
const Dashboard: React.FC<DashboardProps> = ({ lists, currentUser, onSelectKey, onCreateList, onDeleteList, onMoveList, onNavigateAdmin }) => {
  ...
  // Nomenclatura atualizada conforme solicitado
    ...
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      <header className="flex justify-between items-start">
        ...
        {currentUser.role === 'admin' && (
            <button 
                ...
                className="bg-slate-800 text-white p-2 rounded-xl shadow-lg active:scale-95 transition-all"
      ...
      <div className="flex flex-col gap-3">
        {lists.map((list) => (
          <div 
            key={list.id} 
            className="group bg-white rounded-2xl shadow-sm border border-slate-100 transition-all hover:shadow-md overflow-hidden flex flex-col" 
            onClick={() => onSelectKey(list.id)}
          >
             <div className="p-4 flex items-center gap-4 cursor-pointer">
               <div className="bg-slate-50 w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0">
                  {list.icon}
               </div>
         ...
         <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
           <div className="bg-white p-6 rounded-3xl shadow-2xl animate-in zoom-in-95 max-w-xs w-full">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mb-4" 
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button 
                    onClick={() => { onCreateList(newListName, newListIcon); setIsCreating(false); setNewListName('') }}
                    className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-[10px] font-black shadow-lg uppercase"
                >
                    Confirmar
                </button>
              </div>
